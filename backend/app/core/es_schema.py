...
# 전 그룹 단방향(=>) 원칙: 광의어(왼쪽) 검색만 협의어로 확장하고, 협의어 검색은 확장하지 않는다.
# 동치(,) 그룹이면 광의어가 설명문 상투구("주거 안정", "취업역량", "지방보조금")에 걸려
# 협의어 질의를 오염시킴 — 실측(2026-07-14, 코퍼스 2,653건 문서빈도 df):
#   교육 44.6% / 취업 33.7% / 일경험 30.7% / 보조금 27.7% / 창업 20.0% / 일자리 13.7%
#   / 주거 13.3% / 금융 12.2%  ← df 10%+ 단어가 동치 그룹에 있으면 협의어 검색이 수 배~18배 부풂
#   (예: '훈련' 실질 65건이 '교육' 동치 확장으로 1,182건, '월세 지원' 관련외 73건 유입)
# 광의어 판정 기준: df 10% 이상 → 반드시 왼쪽(LHS)으로만. 왼쪽에 여러 단어 가능("a, b => ...").
SYNONYMS = [
...
# ⚠ 등재 금지 목록(df 실측): 지원 90.9% / 청년 75.1% / 사업 51.9% / 운영 32.9% / 신청 11.7%
#   — 이 단어들이 동의어 오른쪽(RHS)에 들어가는 순간 해당 그룹 전체 검색이 코퍼스 절반과 매칭된다.
