...
function BookmarkedCard({
  ...
  if (activeTag !== "전체" && policy.category !== activeTag.replace("#", "")) return null;
...
export default function BookmarkedScreen({ onNavigate }: ScreenProps) {
  const { bookmarks, toggle: toggleBookmark, isBookmarked } = useBookmarkStore();
  ...
  return (
    <div className="flex flex-col h-full bg-white text-slate-800 font-sans select-none overflow-y-auto pt-19">
      ...
      <div className="px-6 pt-5 pb-3 flex items-center justify-between shrink-0">
        ...
        >
          {copied ? (
            ...
            <>
              <svg className="w-3.5 h-3.5" fill="none" strokeWidth="1.5" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex-1 px-6 py-5 space-y-4">
        {bookmarks.length === 0 ? (
          <div className="text-center py-16 flex flex-col items-center gap-2">
            ...
            <p className="text-xs font-bold text-slate-400">찜한 정책이 없습니다.</p>
          </div>
        ) : (
