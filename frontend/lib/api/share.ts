...
// 찜 목록(plcy_no + source) 스냅샷을 서버에 저장하고 공유 코드를 발급받는다. 로그인 불필요.
export async function createShareLink(items: BookmarkItem[]): Promise<ShareCreateResponse> {
...
export async function createShareUrl(items: BookmarkItem[]): Promise<string> {
  const { share_code } = await createShareLink(items);
